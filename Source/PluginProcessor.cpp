...
void NewProjectAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    for (int i = 0; i < 2; i++)
    {
        pDelay[i]->setSampleRate(sampleRate);
        pLFO[i]->setSampleRate(sampleRate);
    }
}
...
void NewProjectAudioProcessor::getStateInformation (juce::MemoryBlock& destData)
{
    ...
    juce::XmlElement* presetBody = new juce::XmlElement("KPPreset");
...
void NewProjectAudioProcessor::initialiseDSP()
{
	for (int i = 0; i < 2; i++)
	{
        pInputGain[i] = std::make_unique<KAPGain>();
        pOutputGain[i] = std::make_unique<KAPGain>();
        pDelay[i] = std::make_unique<KAPDelay>();
        pLFO[i] = std::make_unique<KAPLfo>();
	}
}

juce::AudioProcessorValueTreeState::ParameterLayout NewProjectAudioProcessor::createParam()
{
    std::vector<std::unique_ptr<juce::RangedAudioParameter>> params;
    for (int i = 0; i < kParameter_totalNumParams; i++)
    {
